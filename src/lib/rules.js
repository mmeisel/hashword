...
const rules = {
  ...
  resetRules () {
    ...
    return rules.resetPromise.then(resetRulesStart, resetRulesStart)

    function resetRulesStart () {
      rules.resetPromise = storage.getAll()
        .then(hwData => {
        ...
        chrome.declarativeContent.onPageChanged.removeRules(undefined, () => {
          chrome.declarativeContent.onPageChanged.addRules([addIconRule, insertIconRule], () => {
            if (chrome.runtime.lastError) {
              reject(new Error('Error adding declarativeContent rules: ' +
